Site: https://www.xpdfreader.com/about.html
...
NAME
pdftotext − Portable Document Format (PDF) to text converter (version 4.06)
...
AUTHOR
The pdftotext software and documentation are copyright 1996-2025 Glyph & Cog, LLC.
...
SEE ALSO
NAME
xpdfrc − configuration file for Xpdf tools (version 4.06)
...
discardCoveredText yes | no

If set to "yes", the xpdf tools will discard all text drawn before ("under") a full-page fill. If set to "no", such text will be marked as invisible (only relevant for pdftohtml), but not discarded. This defaults to "no".
...
initialDisplayMode single | continuous | sideBySideSingle |
sideBySideContinuous | horizontalContinuous
...
initialMaximized yes | no

If set to "yes", xpdf windows open maximized. The default is "no".
...
preferXFAFieldValues yes | no

If set to "yes", Xpdf will use the XFA field value if present, and fall back to the AcroForm value otherwise. If set to "no", Xpdf will use the AcroForm value, and completely ignore the XFA value. The default value is "yes".
...
AUTHOR
The Xpdf software and documentation are copyright 1996-2025 Glyph & Cog, LLC.
...
XpdfReader and xpdf (and the command line tools) are configured by way of a plain text configuration file. On Linux, Unix, and Mac, that file is ~/.xpdfrc, i.e., a file called .xpdfrc in your home directory. On Windows, that file is called xpdfrc (no extension), and lives in the same directory as the executables (xpdf.exe).

On all systems, the xpdfrc file must be a plain text file.

For information on the available configuration commands, see the xpdfrc(5) man page.

How do I add (or modify) key bindings in XpdfReader/xpdf?
You can add key bindings in your xpdfrc file (see above), using the bind command (see the xpdfrc(5) man page).

What are the differences between the open source xpdf viewer and the closed source XpdfReader?
XpdfReader has some additional features (available to commercial licensees, but not part of the open source release):

DeviceN (CMYK + spot color) rasterization
color management
Windows printing
How does color management work in XpdfReader?
To enable color management in XpdfReader:

Obtain the ICC profile for your desired output space (RGB or CMYK). Adobe has a collection of profiles here.

Create an xpdfrc file (see above), and add this:

    cmykOutputProfile C:/profiles/cmyk-output-profile.icc
or this:

    rgbOutputProfile C:/profiles/rgb-output-profile.icc
using the path to your profile.

For RGB, you can also use the builtin sRGB profile:

    rgbOutputProfile sRGB
Run XpdfReader and enable color management in the Tools menu.

You may also want to enable DeviceN/overprint preview (also in the Tools menu).

How do I make Xpdf reload a file?
Here are three options:

Xpdf will automatically check for changes to your PDF file whenever you switch pages (via PgUp/PgDn, the scrollbar, etc.).

You can press control-R at any time to reload the file.

You can start Xpdf in remote server mode:

    xpdf -remote foo "openFile(/path/to/file.pdf)"
And then later tell it to reload:

    xpdf -remote foo reload
This option is useful for scripting, for example when running LaTeX.